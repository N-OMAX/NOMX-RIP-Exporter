...
def create_glb(vertices, normals, uvs, faces, textures, dds_data, output_path, model_name):
    try:
        ...
        gltf = {
            "asset": {"version": "2.0", "generator": "NOMX Converter"},
            "scene": 0,
            "scenes": [{"nodes": [0]}],
            "nodes": [{"mesh": 0}],
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("NOMX RIP to OBJ Converter")
        ...
        panel_color = "#1E1E1E"
        text_color = "#E0E0E0"
        accent_color = "#00FF66" # Neon green NOMX vibe
        accent_hover = "#00D655"
        ...
        subtitle = tk.Label(root, text="Convert multiple .RIP files to .OBJ/.MLT seamlessly.", font=("Segoe UI", 10), bg=bg_color, fg="#888888")
        subtitle.pack(pady=(0, 20))

        # Controls Panel
        frame_controls = tk.Frame(root, bg=panel_color, bd=0, highlightthickness=1, highlightbackground="#333333")
        frame_controls.pack(fill=tk.X, padx=25, pady=5, ipady=10)
        ...
        self.lbl_status = tk.Label(frame_controls, text="NO FILES SELECTED", font=("Segoe UI", 10, "bold"), bg=panel_color, fg="#555555")
        self.lbl_status.pack(side=tk.RIGHT, padx=20)

        # Log Area Container
        frame_log = tk.Frame(root, bg=bg_color)
        frame_log.pack(fill=tk.BOTH, expand=True, padx=25, pady=(15, 25))
        
        log_lbl = tk.Label(frame_log, text="TERMINAL OUTPUT //", font=("Consolas", 10, "bold"), bg=bg_color, fg="#555555")
        log_lbl.pack(anchor="w", pady=(0, 5))

        self.text_log = scrolledtext.ScrolledText(frame_log, wrap=tk.WORD, bg="#0A0A0A", fg="#00FF66", font=("Consolas", 10), relief=tk.FLAT, padx=15, pady=15, insertbackground="#00FF66")
        self.text_log.pack(fill=tk.BOTH, expand=True)
        
        # Border for Text Log
        self.text_log.config(highlightbackground="#333333", highlightcolor=accent_color, highlightthickness=1)

        # Redirect terminal output to GUI log
        sys.stdout = self.PrintRedirector(self.text_log)
        sys.stderr = self.PrintRedirector(self.text_log)
    ...
    def select_files(self):
        files = filedialog.askopenfilenames(
            ...
            filetypes=(("RIP Files", "*.rip"), ("All Files", "*.*"))
        )
        if files:
            self.files_to_process = list(files)
            self.lbl_status.config(text=f"{len(self.files_to_process)} FILE(S) READY", fg="#00FF66")
            self.btn_convert.config(state=tk.NORMAL)
            print(f"[*] Loaded {len(self.files_to_process)} file(s) into queue. Ready to export.")

    def start_conversion(self):
        self.btn_select.config(state=tk.DISABLED, bg="#222222", fg="#555555")
        self.btn_convert.config(state=tk.DISABLED, bg="#222222", fg="#555555")
        self.lbl_status.config(text="CONVERTING...", fg="#FF9800")
        self.text_log.delete(1.0, tk.END)
        print("[*] Initializing NOMX Exporter Sequence...")
    ...
    def process_files(self):
        try:
            ...
            print("\n" + "="*50)
            print(" [✓] ALL QUEUED FILES EXPORTED SUCCESSFULLY")
            print("="*50 + "\n")
        except Exception as e:
    ...
    def finish_conversion(self):
        self.btn_select.config(state=tk.NORMAL, bg="#3A3A4A", fg="#FFFFFF")
        self.btn_convert.config(state=tk.NORMAL, bg="#00FF66", fg="#000000")
        self.lbl_status.config(text="EXPORT COMPLETE", fg="#00FF66")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        # Graceful command-line fallback
        try:
        ...
        # Start GUI automatically when launched directly
        root = tk.Tk()
        # Make icon if applicable, otherwise default tk icon
        app = App(root)
        root.mainloop()
